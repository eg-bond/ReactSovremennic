...
async function scrapCinema(id) {
  try {
    ...
    const kindLinksFiltered = Object.values($kindLinks).filter(item => $(item).text() !== "слова")
    ...
    const directorLinksFiltered = Object.values($directorLinks).filter(item => $(item).text() !== "слова")
    const directorLinksArrSliced = Object.values(directorLinksFiltered).slice(0, -4)
    // вытаскиваем innerText из каждой ссылки и формируем строку из жанров, разделенных запятыми
