export const filmsArray = [
  {
    title: 'Годзилла и Конг: Новая империя',
    beginDate: 'с 16 мая',
    endDate: '',
    kind: 'Фантастика, боевик',
    director: 'Адам Вингард',
    duration: '115 мин. / 01:55',
    age: '12+',
    actors:
      'Ребекка Холл, Брайан Тайри Генри, Дэн Стивенс, Кэйли Хоттл, Алекс Фернс, Чэнь Фала, Рэйчел Хаус, Рональд Смик, Чантелл Джемисон, Грег Хаттон',
    description:
      'Два древних титана, Годзилла и Конг, сражаются в эпической битве, пока люди разгадывают их происхождение и связь с тайнами Острова черепа.',
    playerCode: 'nK07u5N7wQ8',
    link: 'godzilla_kong',
  },
  {
    title: 'Кунг-фу Панда 4',
    beginDate: 'с 16 мая',
    endDate: '',
    kind: 'Мультфильм, фэнтези, боевик, комедия, приключения, семейный',
    director: 'Майк Митчелл, Стефани Стайн',
    duration: '94 мин. / 01:34',
    age: '6+',
    actors:
      'Джек Блэк, Сет Роген, Аквафина, Ке Хюи Куан, Брайан Крэнстон, Виола Дэвис, Иэн Макшейн, Дастин Хоффман, Джеймс Хун',
    description:
      'Продолжение приключений легендарного Воина Дракона, его верных друзей и наставника.',
    playerCode: 'Xq7PsTU7vCQ',
    link: 'kung_fu_panda',
  },
  {
    title: 'Майор Гром: Игра',
    beginDate: 'с 23 мая',
    endDate: '',
    kind: 'Боевик, приключения',
    director: 'Олег Трофим',
    duration: '162 мин. / 02:42',
    age: '12+',
    actors:
      'Тихон Жизневский, Александр Сетейкин, Алексей Маклаков, Любовь Аксёнова, Владимир Лысенко, Евгений Чебатков, Ольга Сутулова, Матвей Лыков, Андрей Трушин, Александр Коломиец',
    description:
      'Майор полиции Игорь Гром известен всему Санкт-Петербургу пробивным характером и непримиримой позицией по отношению к преступникам всех мастей. Неимоверная сила, аналитический склад ума и неподкупность — всё это делает майора Грома самым настоящим супергероем. Его жизнь идеальна: днём он ловит преступников вместе с напарником Димой Дубиным, а вечера проводит в компании журналистки Юлии Пчёлкиной. Полную идиллию прерывает появление в городе таинственного злодея, называющего себя Призраком. Он предлагает Грому сыграть в опасную игру, ставка в которой — жизни обычных людей.',
    playerCode: '3kID7Ltwz5g',
    link: 'major_grom',
  },
  {
    title: 'Каскадёры',
    beginDate: 'с 30 мая',
    endDate: '',
    kind: 'Боевик, драма, комедия',
    director: 'Дэвид Литч',
    duration: '126 мин. / 02:06',
    age: '16+',
    actors:
      'Райан Гослинг, Эмили Блант, Аарон Тейлор-Джонсон, Ханна Уэддингхэм, Тереза Палмер, Стефани Сюй, Уинстон Дьюк, Бен Найт, , Адам Данн',
    description:
      'Потрёпанный жизнью каскадёр Колт нехотя возвращается в профессию, чтобы помочь бывшей девушке Джоди снять первый фильм — блокбастер с многомиллионным бюджетом. Ситуация осложняется пропажей исполнителя главной роли.',
    playerCode: 'LoCJN_mixQo',
    link: 'fall_guy',
  },
  {
    title: 'Манюня: Приключения в Москве',
    beginDate: 'с 30 мая',
    endDate: '',
    kind: 'Комедия, приключения',
    director: 'Денис Гуляр',
    duration: '88 мин. / 01:28',
    age: '6+',
    actors:
      'Екатерина Темнова, Карина Каграманян, Карина Мнацаканян, Джульетта Степанян, Евгений Петросян, Сергей Канаев, Андрей Кондратьев, Роман Богданов, Алексей Баранов, Илья Креймер',
    description:
      'Жизнь неугомонной Манюни и ее подружек Каринэ и Наринэ не стоит не месте, и теперь им предстоит отправиться на летние каникулы в Москву. Девочек ждут невероятные события и приключения, а также неожиданные встречи и знакомства.',
    playerCode: 'hVl_xvBYosY',
    link: 'manuna',
  },
  {
    title: 'Непослушники',
    beginDate: 'с 6 июня',
    endDate: '',
    kind: 'Комедия',
    director: 'Владимир Котт',
    duration: '-',
    age: '6+',
    actors:
      'Виктор Хориняк, Гоша Куценко, Юрий Кузнецов, Сергей Селин, Екатерина Новокрещенова, Никита Конкин, Алиса Клагиш, Федор Скальский, Борис Каморзин, Олег Васильков',
    description:
      'Дима — уже не «Димонстр», а вполне успешный менеджер по продажам в автосалоне, Саша Чёрный — добропорядочный монах в тихой обители у отца Анатолия. Все живут спокойно, пока в монастырь не привозят четырех детдомовских «непослушников»: двух братьев и двух сестер. Неоднократные побеги и мелкие кражи приводят к тому, что ребят могут отправить в место куда более суровое, чем детский дом. Следователь не хочет ломать им жизнь и дает последний шанс: привозит их к отцу Анатолию, надеясь, что если тому удалось перевоспитать Димонстра и Сашу Чёрного, то с маленькими детьми тем более получится. Но как же он ошибается…',
    playerCode: '9wjMQHI2fAQ',
    link: 'neposlushniki',
  },
  {
    title: 'Планета обезьян: Новое царство',
    beginDate: 'с 6 июня',
    endDate: '',
    kind: 'Фантастика, боевик, приключения',
    director: 'Уэс Болл',
    duration: '145 мин. / 02:25',
    age: '12+',
    actors:
      'Оуэн Тиг, Питер Макон, Фрейя Аллан, Кевин Дюран, Трэвис Джеффри, Ика Дэрвилл, Лидия Пекхэм, Нил Сэндилэндс, Рас-Сэмюэл, Сара Вайсман, Уильям Х. Мэйси, Дичен Лакмэн',
    description:
      'Несколько поколений после правления Цезаря. Обезьяны являются доминирующим видом, живущим в гармонии, а люди вынуждены оставаться в тени. Пока новый тиранический лидер обезьян строит свою империю, один молодой шимпанзе отправляется в путешествие, которое заставит его усомниться во всём, что он знал о прошлом, и сделать выбор, который определит будущее как обезьян, так и людей.',
    playerCode: 'XhtjITgi69I',
    link: 'apes',
  },
  {
    title: 'Заветное желание',
    beginDate: 'с 7 июня',
    endDate: '',
    kind: 'Мультфильм, мюзикл, фэнтези, комедия, приключения, семейный',
    director: 'Крис Бак, Фон Вирасунторн',
    duration: '95 мин. / 01:35',
    age: '6+',
    actors:
      'Ариана Дебоуз, Крис Пайн, Алан Тьюдик, Анжелика Кебрал, Виктор Гарбер, Наташа Ротуэлл, Дженнифер Кумияма, Харви Гильен, Нико Варгас, Эван Питерс',
    description:
      'Аша загадывает настолько сильное желание, что на него отвечает космическая сила — маленький шарик безграничной энергии по имени Звезда. Вместе они противостоят самому грозному правителю Росаса, королю Магнифико.',
    playerCode: 'UV4n_hy2Tl4',
    link: 'wish',
  },
  {
    title: '10 жизней',
    beginDate: 'с 12 июня',
    endDate: '',
    kind: 'Мультфильм, комедия, семейный, фэнтези',
    director: 'Крис Дженкинс',
    duration: '88 мин. / 01:28',
    age: '6+',
    actors:
      'Мо Гиллиган, Симон Эшли, Софи Оконедо, Зейн Малик, Дилан Ллевелин, Джереми Свифт, Билл Найи',
    description:
      'Все прекрасно знают, что у котов девять жизней. А еще говорят, что коты ленивые, капризные и довольно агрессивны к окружающим. И все-таки даже самые заносчивые из них просто душки по сравнению с пушистой бестией по имени Беккет. Но его будни скоро изменятся. Притом, целых десять раз! Коту предстоит побывать в шкуре совершенно разных животных, чтобы, наконец, начать ценить свою собственную жизнь.',
    playerCode: 'SM78OH70j0s',
    link: '10_lives',
  },

  {
    title: 'Фуриоса: Хроники Безумного Макса',
    beginDate: 'скоро',
    endDate: '',
    kind: 'Фантастика, боевик, триллер, приключения',
    director: 'Джордж Миллер',
    duration: '148 мин. / 02:28',
    age: '18+',
    actors:
      'Крис Хемсворт, Аня Тейлор-Джой, Алила Браун, Том Бёрк, Нэйтан Джонс, Джош Хелман, Чарли Фрейзер, Лейчи Халм, Джон Ховард, Джордж Шевцов',
    description: 'О молодости бесстрашной воительницы Фуриосы.',
    playerCode: 'yWWZgiHQMT8',
    link: 'furiosa',
  },
  {
    title: 'Гарфилд',
    beginDate: 'скоро',
    endDate: '',
    kind: 'Мультфильм, фэнтези, комедия, приключения, семейный',
    director: 'Марк Диндал',
    duration: '101 мин. / 01:41',
    age: '6+',
    actors:
      'Крис Пратт, Сэмюэл Л. Джексон, Ханна Уэддингхэм, Винг Реймз, Николас Холт, Сесили Стронг, Харви Гильен, Бретт Голдстин, Бовэнь Ян, Снуп Догг',
    description:
      'После неожиданного воссоединения со своим давно потерянным отцом — уличным котом Виком — Гарфилд вместе с другом-псом Оди вынужден отказаться от своей идеально тепличной жизни и присоединиться к весёлому и опасному приключению.',
    playerCode: 'hq2yOIHilDI',
    link: 'garfield',
  },
  {
    title: 'Три богатыря. Ни дня без подвига',
    beginDate: 'с 20 июня',
    endDate: '',
    kind: 'Мультфильм, приключения, фэнтези',
    director: 'Александра Шоха, Екатерина Салабай',
    duration: '-',
    age: '6+',
    actors:
      'Олег Куликович, Валерий Соловьев, Дмитрий Быковский-Ромашов, Сергей Маковецкий, Дмитрий Высоцкий, Лия Медведева, Мария Цветкова-Овсянникова, Анатолий Петров, Юлия Зоркина, Яков Культиасов',
    description:
      'У трех богатырей что ни день, то подвиг по расписанию. То придется расследовать похищение княжеской короны, то спасать друзей из волшебного мира снов, то Конь Юлий с Алешей схлестнутся в эпической битве умов. Не бывает маленьких приключений, да и богатыри наши — немаленькие.',
    playerCode: 'RJbl-Sqe7U8',
    link: 'bogatyri',
  },
  {
    title: 'Тур с Иванушками',
    beginDate: 'с 20 июня',
    endDate: '',
    kind: 'Комедия, музыка',
    director: 'Марк Горобец',
    duration: '108 мин. / 01:48',
    age: '16+',
    actors:
      'Агата Муцениеце, Анна Пескова, Ольга Сухарева, Александрина Олексюк, Роман Маякин, Роман Курцын, Кирилл Андреев, Андрей Григорьев-Аполлонов, Кирилл Туриченко, Татьяна Догилева',
    description:
      'Три подруги, в детстве фанатевшие от «Иванушек International», понимают, что за всю жизнь не совершили ни одного безумного поступка, всегда всё делали ради кого-то, а не для себя. Они решают исполнить свою детскую мечту — поехать в тур за «Иванушками» по югу России, чтобы наконец-то отвлечься от всех проблем.',
    playerCode: 'FrOIjuTV4vk',
    link: 'ivans',
  },
]
